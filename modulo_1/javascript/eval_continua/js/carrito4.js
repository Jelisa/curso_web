...
function addProduct(productName) {
    ...
    //Si la cantidad es correcta.
        ...
        let newProduct = new Product(productName, quantity, getProductPrice(document.getElementById(productName).innerHTML))
        //Se actualiza el carrito de la compra.
        shoppedProducts.push(newProduct)
        console.log(0, shoppedProducts)
        updateShoppingCart();
    }
...
function removeShoppingOptions() {
    ...
    for (x of document.getElementsByClassName("imatges")) x.onclick = function () { };
    for (x of document.getElementsByClassName("delete_button")) x.remove();
}
...
function updateShoppingCart() {
    ...
    for (let i = 0; i< shoppedProducts.length; i++){
        shoppingcartElements += `<p class="shopping_list_element">${generateShoppingCartString(shoppedProducts[i], i)} <button class="delete_button" onclick="delete_element(${i})">X</button></p>`;
...
function generateShoppingCartString(product, index) {
    return `${product.name}  ${product.amount} ${product.units} x ${product.floatToStringWithComa(product.unitPrice)}€/${product.units} = ${product.floatToStringWithComa(product.subtotal)}€`;
    // función que convierte el objeto en una string con el formato necesario para el carrito de la compra
    // usamos la longitud de la array de elementos comprados -1 como indice para eliminar el producto.
}
